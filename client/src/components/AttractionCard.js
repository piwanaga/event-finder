...
const AttractionCard = ({ name, img, id, eventCount }) => {
    ...
    return (
        <div className='relative w-full group h-28 overflow-hidden sm:h-44 md:h-32 xl:h-48'>
            <Link to={`/attraction/${id}`}>
                <img src={img} alt={`${name}`} className='h-full w-full rounded object-cover transition-transform duration-500 ease-in-out transform group-hover:scale-110' />
